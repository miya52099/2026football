...
export async function fetchFreeFixtures(): Promise<WorldCupData> {
  ...
  if (url) {
    console.log(`[NextJS FreeFixtures] Fetching fixtures from custom URL: ${url}`);
    ...
    try {
      ...
      clearTimeout(timeoutId);
      console.warn("[NextJS FreeFixtures] Custom URL fetch failed, falling back to local file:", err.message || err);
    }
  }
